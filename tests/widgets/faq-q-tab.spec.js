...
test("Faq Q Tab - Snapshot", async ({ page }) => {
  await safeGoto(page, `${BASE_URL}/faq-q-tab/`);
  const content = page.locator("main#content");
  await expect(content).toBeVisible({ timeout: 10000 });
  await expect(content).toMatchAriaSnapshot(`
    - main:
      - heading "FAQ Q Tab" [level=1]
      - heading "Random Queries" [level=1]
      - text: Shipping & Delivery Refund & Exchange
      - img
      - text: MSF
      - list:
        - listitem:
